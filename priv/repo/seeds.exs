# Seeds should only run in dev environment, not in test
# Test data should be created via fixtures in tests
unless Mix.env() == :test do
    ...
    case Companies.get_company_by_user(user) do
      nil ->
        ...
        case Companies.create_company(scope, attrs) do
          ...
          {:error, changeset} ->
            IO.puts(
              "Failed to create company for user #{user.email}: #{inspect(changeset.errors)}"
            )
  ...
  more_tech_jobs =
    for _idx <- 6..50 do
      ...
      position = Enum.random([:Employee, :"Specialist Role", :"Leadership Position"])
  ...
  existing_tech_jobs =
    Repo.all(from j in JobPosting, where: j.company_id == ^company1.id, limit: 1)
