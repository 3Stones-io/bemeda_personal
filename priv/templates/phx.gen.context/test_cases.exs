...
describe "list_<%= schema.plural %>/0" do
  test "returns all <%= schema.plural %>", async: true do
...
describe "get_<%= schema.singular %>!/1" do
  test "returns the <%= schema.singular %> with given id", async: true do
    <%= schema.singular %> = <%= schema.singular %>_fixture()
...
describe "create_<%= schema.singular %>/1" do
  test "with valid data creates a <%= schema.singular %>", async: true do
  ...
  test "with invalid data returns error changeset", async: true do
...
describe "update_<%= schema.singular %>/2" do
  test "with valid data updates the <%= schema.singular %>", async: true do
    <%= schema.singular %> = <%= schema.singular %>_fixture()
...
describe "delete_<%= schema.singular %>/1" do
  test "deletes the <%= schema.singular %>", async: true do
    <%= schema.singular %> = <%= schema.singular %>_fixture()
...
describe "change_<%= schema.singular %>/1" do
  test "returns a <%= schema.singular %> changeset", async: true do
    <%= schema.singular %> = <%= schema.singular %>_fixture()
