defmodule BemedaPersonalWeb.UserAuth do
  ...
  ## `on_mount` arguments
    ...
    * `:require_complete_profile` - Requires user to be authenticated and
      have a complete profile (first_name, last_name, department, medical_role).
  ...
  the `current_scope`:
      ...
      defmodule BemedaPersonalWeb.PageLive do
        ...
        on_mount {BemedaPersonalWeb.UserAuth, :mount_current_scope}
        ...
      end
  ...
  def on_mount(:require_sudo_mode, _params, session, socket) do
    ...
    if Accounts.sudo_mode?(socket.assigns.current_scope.user, -10) do
      ...
      socket =
        ...
        |> Phoenix.LiveView.put_flash(
          :error,
          dgettext("auth", "You must re-authenticate to access this page.")
        )
        |> Phoenix.LiveView.redirect(to: ~p"/users/log_in")
  ...
  def on_mount(:require_admin_user, params, _session, socket) do
    ...
    if company.admin_user_id == socket.assigns.current_scope.user.id do
  ...
  def on_mount(:require_complete_profile, _params, session, socket) do
    socket = mount_current_scope(socket, session)

    if socket.assigns.current_scope && socket.assigns.current_scope.user do
      user = socket.assigns.current_scope.user

      if (user.user_type == :job_seeker && !user.profile && !user.work_profile) or
           user.user_type == :employer  && !user.profile do
            socket =
              socket
              |> Phoenix.LiveView.put_flash(
                :info,
  ...
  def assign_current_scope(conn, _opts) do
    ...
    if user_token do
      {user, _token_inserted_at} =
        Accounts.get_user_by_session_token(user_token) || {nil, nil}
  ...
  def require_admin_user(conn, _opts) do
    ...
    if company.admin_user_id == conn.assigns[:current_scope].user.id do
      conn
    else
      conn
      |> put_flash(
  ...
  defp signed_in_path_for_user(%Accounts.User{user_type: :employer} = user) do
  ...
  defp signed_in_path_for_user(%Accounts.User{user_type: :job_seeker} = user) do
    if user.profile && user.work_profile do
      ~p"/"
    else
      ~p"/users/profile"
    end
