      ...
      >
        <div class="w-full max-w-[28rem] lg:max-w-[34rem] px-4 md:px-0">
          <div class="font-medium text-2xl text-[#1f1f1f] text-center mb-8">
            <span>{dgettext("emails", "Join as a")}</span>
            <span :if={@account_type == "employer"}>{dgettext("auth", "employer")}</span>
            <span :if={@account_type == "job_seeker"}>{dgettext("auth", "job seeker")}</span>
          </div>
