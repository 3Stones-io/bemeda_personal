defmodule BemedaPersonal.Jobs do
  ...
  @doc """
  Finds a job posting by Mux upload ID.
  Returns nil if no job posting is found.

  ## Examples

      iex> find_job_posting_by_upload_id("upload_123")
      %JobPosting{}

      iex> find_job_posting_by_upload_id("nonexistent")
      nil

  """
  @spec find_job_posting_by_upload_id(String.t()) :: job_posting() | nil
  ...
  @doc """
  Finds a job posting by Mux asset ID.
  Returns nil if no job posting is found.

  ## Examples

      iex> find_job_posting_by_asset_id("asset_123")
      %JobPosting{}

      iex> find_job_posting_by_asset_id("nonexistent")
      nil

  """
  @spec find_job_posting_by_asset_id(String.t()) :: job_posting() | nil
  def find_job_posting_by_asset_id(asset_id) when is_binary(asset_id) do
    query =
      from jp in JobPosting,
        where: fragment("(?->>'mux_data')::jsonb->>'asset_id' = ?", jp, ^asset_id),
        limit: 1
