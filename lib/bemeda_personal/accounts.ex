defmodule BemedaPersonal.Accounts do
  ...
  """
  @spec update_user_email(user(), token()) :: {:ok, user()} | {:error, any()}
  def update_user_email(user, token) do
    ...
    Repo.transact(fn ->
      with {:ok, query} <- UserToken.verify_change_email_token_query(token, context),
        ...
        _reason -> {:error, :transaction_aborted}
      end
    end)
  end
  ...
  @spec update_user_password(user(), attrs()) ::
          {:ok, {user(), [UserToken.t()]}} | {:error, changeset()}
  ...
  @spec login_user_by_magic_link(token()) ::
          {:ok, {user(), [UserToken.t()]}} | {:error, :not_found}
  ...
  """
  @spec deliver_login_instructions(user(), function()) :: {:ok, map()} | {:error, any()}
  ...
  def delete_user_session_token(token) do
    query = from(t in UserToken, where: t.token == ^token and t.context == "session")
  ...
  def update_user_personal_info(%User{} = user, attrs) do
    user
    |> change_user_personal_info(attrs)
    |> Repo.update()
  end
  ...
  def change_user_personal_info(%User{} = user, attrs \\ %{}) do
    if user.user_type == :job_seeker do
      User.job_seeker_personal_info_changeset(user, attrs)
    else
      User.employer_personal_info_changeset(user, attrs)
    end
  end
end
