defmodule Mix.Tasks.Phx.Gen.BemedaLive do
  ...
  Alternatively, the `--context-app` option may be supplied to the generator:
      ...
      mix phx.gen.bemeda_live Accounts User users --context-app warehouse
  ...
  module name, for example:
      ...
      mix phx.gen.bemeda_live Accounts User users --web Sales
  ...
  for file generation control.
      ...
      mix phx.gen.bemeda_live Accounts User users --no-context --no-schema
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix phx.gen.bemeda_live must be invoked from within your *_web application root directory"
      )
    end

    Mix.Phoenix.ensure_live_view_compat!(__MODULE__)

    # Use our custom BemedaContext module instead of the default Context module
    {context, schema} = Gen.BemedaContext.build(args)
    Gen.BemedaContext.prompt_for_code_injection(context)
  ...
  defp context_files(%Context{generate?: true} = context) do
    Gen.BemedaContext.files_to_be_generated(context)
  end
  ...
  defp copy_new_files(%Context{} = context, binding, paths) do
    ...
    if context.generate?, do: Gen.BemedaContext.copy_new_files(context, paths, binding)
  ...
  def print_shell_instructions(%Context{schema: schema, context_app: ctx_app} = context) do
    ...
    if context.generate?, do: Gen.BemedaContext.print_shell_instructions(context)
