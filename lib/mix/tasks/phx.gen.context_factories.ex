defmodule Mix.Tasks.Phx.Gen.ContextFactories do
  ...
  def copy_new_files(%Context{schema: schema} = context, paths, binding) do
    ...
    # Skip fixture generation since we're using factories
    # inject_test_fixture(context, paths, binding)

    # Generate factory for the schema
    ...
    factory_binding = binding ++ [
      ...
      aliases: [inspect(schema_module)]
    ]
    ensure_factory_file_exists(paths, factory_binding)
    inject_factory_function(schema, paths, factory_binding)
  ...
  defp inject_factory_function(schema, paths, binding) do
    ...
    unless factory_function_exists?(factory_file, schema) do
      ...
      inject_eex_before_final_end(factory_function, factory_file, binding)
    end
  end
  ...
  def ensure_test_file_exists(%Context{test_file: test_file} = context, paths, binding) do
    unless Context.pre_existing_tests?(context) do
      # Create the test file without factory imports since it's available from data_case
      Mix.Generator.create_file(
        test_file,
  ...
  defp inject_tests(%Context{test_file: test_file} = context, paths, binding) do
    ...
    # Get the schema from binding
    schema = Keyword.get(binding, :schema)

    # Update the test file to use factories instead of fixtures
    test_file_content = File.read!(test_file)
    updated_content = test_file_content
      # Replace fixture references with factory calls, but don't add the import
      |> String.replace("#{schema.singular}_fixture()", "insert(:#{schema.singular})")

    File.write!(test_file, updated_content)

    # Generate test cases using factories
    test_cases = Mix.Phoenix.eval_from(paths, "priv/templates/phx.gen.context/test_cases.exs", binding)

    # Replace any remaining fixture references with factory calls
    test_cases = String.replace(test_cases, "#{schema.singular}_fixture()", "insert(:#{schema.singular})")

    inject_eex_before_final_end(test_cases, test_file, binding)
  end
