defmodule Credo.Check.Readability.DepsOrder do
  @moduledoc """
  ...
  # Extract dependencies from different AST formats
  defp extract_deps(list) do
    Enum.flat_map(list, fn
      # Tuple style: {:package, "version"}
      {:{}, meta, [name | _rest]} ->
        [{meta[:line] || 0, to_string(name)}]

      # Keyword style: package: "version"
  ...
  defp check_alphabetical_order(deps, function_name, issue_meta) do
    ...
    |> Enum.filter(fn [{_line1, current_dep}, {_line2, next_dep}] ->
      String.downcase(current_dep) > String.downcase(next_dep)
    end)
    |> Enum.map(fn [{line_no, dep_name} | _rest] ->
      issue_for(issue_meta, line_no, dep_name, function_name)
    end)
  end

  defp issue_for(issue_meta, line_no, trigger, function_name) do
    format_issue(
      issue_meta,
      message: "The dependency #{trigger} in #{function_name}/0 is not alphabetically ordered.",
      trigger: trigger,
      line_no: line_no,
      column: 1
    )
