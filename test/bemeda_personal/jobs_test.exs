defmodule BemedaPersonal.JobsTest do
  ...
  describe "create_job_application/3" do
    ...
    test "creates chat messages when job application is created" do
      ...
      messages = Jobs.list_messages(application_with_cover)
      ...
      video_messages = Jobs.list_messages(application_with_video)
  ...
  describe "get_message!/1" do
    ...
    test "returns the message with given id", %{
      ...
      result = Jobs.get_message!(message.id)
    ...
    test "raises error when message does not exist" do
      non_existent_id = Ecto.UUID.generate()
      assert_raise Ecto.NoResultsError, fn -> Jobs.get_message!(non_existent_id) end
    end
  end
  ...
  describe "create_message/3" do
    ...
    test "with valid data creates a message", %{
      ...
      assert {:ok, %Jobs.Message{} = message} =
               Jobs.create_message(user, job_application, valid_attrs)
  ...
  describe "delete_message/1" do
    ...
    test "successfully deletes the message", %{
      ...
      assert {:ok, %Jobs.Message{}} = Jobs.delete_message(message)
      assert_raise Ecto.NoResultsError, fn -> Jobs.get_message!(message.id) end
    end
  end
  ...
  describe "change_message/2" do
    ...
    test "returns a message changeset", %{
      ...
      assert %Ecto.Changeset{} = Jobs.change_message(message)
    end
